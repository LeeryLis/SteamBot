...
from tools.file_managers import ManualTradeItemManager
...
class ConsoleManualTradeItemManager(BasicConsole):
    def __init__(self, original: ManualTradeItemManager):
        self.original = original
        self.console = Console()

    @command(
        aliases=["add", "add_item"],
        description="Add the item to the manual trade items file",
...
if __name__ == "__main__":
    while not is_str_int(app_id := input("Введите ID игры: ")):
        print("Неверный ввод")
    ConsoleManualTradeItemManager(ManualTradeItemManager(int(app_id))).run("ManualTradeItemManager")
